...
public interface StopJpaRepository extends CommonJpaRepository<StopEntity, Long>,
   ...
   // Collection<StopEntity> searchByName(String name);

   /* Collection<StopEntity> findDistinctByNameAndActiveIsTrue(String name);
...
//    countQuery = "select   count(v) from  VehicleEntity v")  // через имена, но нужно доуказать @Param
    ...
    // countQuery пишем только в том случае, когда у нас Page
